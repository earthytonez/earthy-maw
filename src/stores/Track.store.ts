...
export default class TrackStore {
    ...
    addTrack() {
        this.tracks.push(new Track(this.tracks.length, this.audioContext, this.rootStore.musicFeaturesStore));
        ...
        const loadTracks = async () => {      
            if (tracksFromLocalStore && tracksFromLocalStore.length > 0) {
              let trackObjects: Track[] = await pMap(
                ...
                async (trackData, i) => {
                  let t = new Track(i, this.audioContext, this.rootStore.musicFeaturesStore);
